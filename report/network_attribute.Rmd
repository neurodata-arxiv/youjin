...
where $A^{H} = HAH; A_{ij} = \parallel X_{i} - X_{j} \parallel ; H = I_{n} - J_{n} / n.$ Sample distance correlation is obtained through normalization. It is known that as $n \rightarrow \infty,$ the sample distance correlation, called dCor coverages to 0 if and only if $X$ and $Y$ are independent each other. Unfortunately, however, simulation results show that dCor performs worse if $X$ or $Y$ is high-dimensional. Now we suggest using \textit{local} distance covariance instead:
...
(2) $B = 2;$ $\rho = (0.0, 0.2)$; ratio = (1,1); $n=300$ ; $M=500$



<hr />

(3) $B =2;$ $\rho = (0.1, 0.2);$ raio = (1,1); $n=300$; $M=500$
...
```{r echo = FALSE, results='asis'}
dblock4 <- c( 0.1000000, 0.1133333, 0.1200000, 0.1666667)
maxiblock4 <- c(0.1166667, 0.1400000, 0.1600000, 0.1733333)
