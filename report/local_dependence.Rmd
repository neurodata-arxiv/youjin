...
$$A = \left[  \begin{array}{c|c | c} p & q + \epsilon & q \\ \hline q + \epsilon & p & q \\ \hline q & q & p \end{array} \right]$$

Set $n=150$ ; $p = 0.3$ ; $q = 0.05;$ $\omega = 0.15$;  $\epsilon = 0.05$. In other words, the probability of having edges between or within block is:

$$A = \left[  \begin{array}{c|c| c} 0.3 & 0.1 & 0.05 \\ \hline 0.1 & 0.3 & 0.05 \\ \hline 0.05 & 0.05 & 0.3 \end{array} \right]$$

```{r, out.width = 500, out.height = 500, echo = FALSE, fig.align='center'}
knitr::include_graphics("../figure/local11_net.png")
```
...
```{r, out.width = 200, out.height = 300, echo = FALSE, fig.align='center', fig.show='hold', out.extra='style="float:left"'}
knitr::include_graphics("../figure/local11_1.png")
```
```{r, out.width = 200, out.height = 300, echo = FALSE, fig.align='center', fig.show='hold', out.extra='style="float:left"'}
knitr::include_graphics("../figure/local11_2.png")
```
```{r, out.width = 200, out.height = 300, echo = FALSE, fig.align='center', fig.show='hold', out.extra='style="float:left"'}
knitr::include_graphics("../figure/local11_5.png")
```
```{r, out.width = 200, out.height = 300, echo = FALSE, fig.align='center', fig.show='hold'}
knitr::include_graphics("../figure/local11_10.png")
```


```{r echo = FALSE, results='asis'}
a <- c(0.6166667, 0.5800000, 0.5500000, 0.5333333)
...
As diffusion time increases, estimated power from global test is decreasing while that from the local optimal increases. That might be because the probability of journey from ``Block1`` to ``Block3`` is higher than ``Block1`` to ``Block2``, but the distribution between ``Block1`` and ``Block2`` is more similar. When the number of block is two, if $p > q$, global test achieves highest power because the distance between blocks is always proportion to the discrepancy in distributions.
