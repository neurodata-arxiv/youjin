...
#### * Explictly formalize our testing goal 
  ...
  Different from an adjacency matrix, diffusion maps at each time point $\{ \boldsymbol{U}_{t}(i) \}_{i \in \{ 1,2,...,n \}}$ provides a linearly independent representation of each node and Euclidean distance between them is equivalent to diffusion distance, which is a well-defined measure for connectivity. Even in the case where $\boldsymbol{U}_{t}(i) \in q < n$, we have nice properties in testing independence.
 
 Assume we have a random graph $\mathbf{G} \in \mathcal{G}$ of $n \in \mathbb{N}$ vertices. Derive or observe $n$ pairs of $\{ (\mathbf{U}_{t}(i), \mathbf{X}_{i}) : t \in \mathbb{N}, i = 1,2,... , n\}$, where $\mathbf{U}_{t} \in \mathbb{R}^{q} \leq n$ and $\mathbf{X} \in \mathbb{R}^{m}$. 
...
<a name = "Theorem2"/>
<b> $$\mbox{Theorem 2}$$  <center>  Assume $E\big| \boldsymbol{U}^2 \big| < \infty$   and $E\big| \boldsymbol{X}^2 \big| < \infty$. For a fixed $t$, $P[ \mathcal{T}_{n} < t ] \rightarrow P[t_{\nu - 1} < t]$ as $q,m \rightarrow \infty$
