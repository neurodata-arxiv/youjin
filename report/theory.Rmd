...
A transition matrix $P$ is a new kernel of a Markov chain of which element $P[i,j]$ represents the probability of travel from Node $i$ to Node $j$ in one time step. On the other hand, corresponding probability in $t$ steps is given by the $t$ th ($t \in \mathbb{N}$) power of $P$. How to derive diffustion distance over a directed network or weighted network is provided in [Tang & Trosset (2010)](#Tang). Other than a transition matrix, we need a stationary a stationary probability $\boldsymbol{\pi} = \{\pi(1), \pi(2), ... , \pi(n) \}$ of which $\pi(i)$ denotes the probability that the chain stays in Node $i$ regardless of the starting state. In our setting, $\pi(i)$ is proportional to the degree of Node $i$, i.e. $\pi(i) = \sum\limits_{j=1}^{n} A_{ij} \big/ \sum\limits_{i=1}^{n}\sum\limits_{j=1}^{n} A_{ij}$ ($i=1,2,..., n$).   
...
<a name = "Theorem1"/>
<b> $$\mbox{Theorem 1}$$  
<center> Assume that an adjacency matrix $\mathbf{A}$ for a given graph $\mathbf{G}$ is invertible. Then for a fixed $t$, $\{ \boldsymbol{U}_{t}(1), \boldsymbol{U}_{t}(2), ... , \boldsymbol{U}_{t}(n) \}$ provides a linearly independent $n$-coordinate vectors.
</center></b>
...
<a name = "Theorem2"/>
<b> $$\mbox{Theorem 2}$$  <center> Preserve all the notations and assumptions made in [Theorem 1](#Theorem1). Assume $E\big| \boldsymbol{U}^2 \big| < \infty$   and $E\big| \boldsymbol{X}^2 \big| < \infty$. For a fixed $t$, $\mathcal{R}_{n}(\boldsymbol{U}, \boldsymbol{X})$ (MCorr)  = 0 if and only if $\boldsymbol{U}$ and $\boldsymbol{X}$ are independent, i.e. connectivity at $t$ between nodes are independent of their attributes.
</center></b> 


Details are provided in [Appendix](# Appendix). This Theorem is followed by assumption that our observed $\boldsymbol{G}$ is a randomly sampled network so that $\{ (\boldsymbol{U}_{t}(i), \boldsymbol{X}_{i} ) : i = 1,2, ... , n \}_{t \in \mathbb{N}}$ is a random sample from the joint distribution of $\{ (\boldsymbol{U}_{t}, \boldsymbol{X})\}_{t \in \mathbb{N}}$ from population network.
...
#### * Proof of linear independence of diffusion maps ([Theorem 1](#Theorem1)).
  ...
  Assume that a unweighted and undirected, connected network $\boldsymbol{G}$ has a finite number of vertices $n \in \mathbb{N}$. Connectiveness of $\boldsymbol{G}$ leads to non-zero stationary probabilities $\{ \pi(i) > 0 : i =1, ... , n \}$. Also its adjacency matrix $\boldsymbol{A}$ is a symmetric but its transition matrix $\boldsymbol{P}[i,j] = A_{ij} \big/ \sum\limits_{j=1}^{n} A_{ij} = A_{ij} \big/ \pi(i)$ does not guarantee its symmetry. Define a symmetric kernel $\boldsymbol{Q} = \boldsymbol{\Pi^{1/2} P \Pi^{-1/2}}$ instead, where $\boldsymbol{\Pi}$ is a $n \times n$ diagonal matrix of which $i$th diagonal element of $\pi(i)$. That is, for any $i,j = \{1,2,... , n \}:$ 
  
  $$Q[i,j] = \frac{\sqrt{\pi(i)}}{\sqrt{\pi(j)}} \frac{A_{ij}}{\pi(i)} = \frac{A_{ij}}{\sqrt{\pi(i)} \sqrt{\pi(j)}}.$$
...
Throughout this paper, we assume that our observed network $\boldsymbol{G}$ is a randomly sampled network, denoted by $\mathbf{\mathbb{G}} = \big( \mathbf{\mathbb{A}}, \mathbf{\mathbb{X}}  \big)$. A $n \times n$ matrix $\mathbb{A}$ is an adjacency matrix of $\mathbf{\mathbb{G}}$ and $m$-variate vector $\mathbf{\mathbb{X}}_{i}$ denotes each node's attribute values $(i=1,2,...,n)$. 


Fix a diffusion time $t > 0$. Recall that a set of diffusion distance $\boldsymbol{U}_{t} = \{ \boldsymbol{U}_{t}(1) , \boldsymbol{U}_{t}(2) , ... , \boldsymbol{U}_{t}(n)  \}$ are constructed from an adjacency matrix so that $\boldsymbol{U}_{t} = f(\boldsymbol{A})$. If we let $\mathbf{\mathbb{U}}_{t}$ be a diffusion map defined from $\mathbf{\mathbb{A}}$, since $\boldsymbol{A}$ is a random subgraph of $\mathbf{\mathbb{A}}$, $\boldsymbol{U}_{t}$ is also a random diffusion map from $\mathbf{\mathbb{A}}$, i.e. $\boldsymbol{U}_{t}$ is a random observation of $\mathbf{\mathbb{U}_{t}}$. 

[Theorem 1](#Theorem1) supports the independence of $\boldsymbol{U}_{t}$. Thus we observe $n$-pair of observations $\{ ( \boldsymbol{U}_{t}(i), \boldsymbol{X}_{i} ) :  \boldsymbol{U}_{t}(i) \in \mathbb{R}^{n}, \boldsymbol{X} \in \mathbb{R}^{n}, i = \{ 1,2, ... , n \}, t \in \mathbb{N}  \}$ for each vertex in $\boldsymbol{G}$. Then under the provided assumptions, ``dCorr`` (or ``MCorr``) defined on these pairs of observations are measuring distance between the joint characteristic function $f_{U,X}$ of $\mathbf{U}$ and $\mathbf{X}$ and the the product $f_{U}$ $f_{X}$ of the marginal characteristic functions for each $\mathbf{U}$ and $\mathbf{X}$ ([Szekely and Rizzo, 2013](Szekely2)).  
