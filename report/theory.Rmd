...
[Theorem 1](#Theorem1) supports the independence between each column of $\boldsymbol{U}_{t}$. Thus we observe $iid$ $n$-pair of observations $\{ ( \boldsymbol{U}_{t}(i), \boldsymbol{X}_{i} ) :  \boldsymbol{U}_{t}(i) \in \mathbb{R}^{n}, \boldsymbol{X} \in \mathbb{R}^{n}, i = \{ 1,2, ... , n \}, t \in \mathbb{N}  \}$ for each vertex in $\boldsymbol{G}$. Then under the provided assumptions, ``dCorr`` (or ``MCorr``) defined on these pairs of observations are measuring distance between the joint characteristic function $f_{UX}$ of $\mathbf{U}$ and $\mathbf{X}$ and the the product $f_{U}$ $f_{X}$ of the marginal characteristic functions for each $\mathbf{U}$ and $\mathbf{X}$ ([Szekely and Rizzo, 2013](Szekely2)).  $\blacksquare$ 
