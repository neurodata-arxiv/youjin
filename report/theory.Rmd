...
$$C^2_{t}[i,j] = \sum\limits_{w =1}^{n} \big( P^{t}[i,w] - P^{t}[j,w]  \big)^{2} \frac{1}{\pi(w)} = \sum\limits_{w=1}^{n} \left(  \frac{P^{t}[i,w]}{\sqrt{\pi(w)}} - \frac{P^{t}[j,w]}{\sqrt{\pi(w)}}   \right)^2 = \parallel \frac{P^{t}[i, \cdot]}{\sqrt{\boldsymbol{\pi} }} - \frac{P^{t}[j, \cdot]}{\sqrt{\boldsymbol{\pi}}}  \parallel^2$$
