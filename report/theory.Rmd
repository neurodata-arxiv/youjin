...
#### * Spectral properties of diffusion maps
  ...
  Diffusion distance of $\boldsymbol{G}$ defined as above can be represented via a spectral decomposition of its transition matrix $P$. That is, we can derive diffusion distance using its eigenvectors and eigenvalues. Recall that $C_{t}$ is a functional $L^2$ distance, weighted by 1/$\pi$. If we transform the way to represent $C_{t}[i,j]$ a little, we are able to obtain an orthonomal basis of $L^{2}(\mathbb{G}, d\mu / \pi)$ via eigenvalues and eigenvectors. 
...
The advantages from such representation is that it is now possible to represent given network $\boldsymbol{G}$ as a system of $n$ coordinates called diffusion coordinate, of which metric well reflects how corresponding vertices are connected each other at each diffusion time point. That is, for a fixed $t$, $\{ \boldsymbol{U}_{t}(1), \boldsymbol{U}_{t}(2), ... , \boldsymbol{U}_{t}(n) \}$ can be assumed to be iid observations. Linear independence of $\{\boldsymbol{U}_{t}(i)\}_{i \in \{1,2,...,n\}}$ is proven in [Appendix](# Appendix).
...
$$\boldsymbol{U}_{t}(i) = \begin{pmatrix} \lambda^{t}_{1} \phi_{1}(i) \\ \lambda^{t}_{2} \phi_{2} (i)  \\ \vdots \\ \lambda^{t}_{n} \phi_{n}(i) \end{pmatrix} =  \begin{pmatrix} \frac{\lambda^{t}_{1}}{\sqrt{\pi(1)}} \psi_{1}(i) \\ \frac{\lambda^{t}_{2}}{\sqrt{\pi(2)}} \psi_{2} (i)  \\ \vdots \\ \frac{\lambda^{t}_{n}}{\pi(n)} \psi_{n}(i) \end{pmatrix}  \in \mathbb{R}^{n}.$$
...
Then diffusion map matrix of $\boldsymbol{U}_{t}$ at $t$ of which each column denotes Node $i$'s diffusion map can be represented as $\boldsymbol{U}_{t} = \Lambda^{t} \Pi^{-1/2} \Psi$. Note that each of $\Lambda^{t}$, $\Pi^{-1/2}$, and $\Psi$ are invertible, so $\boldsymbol{U}_{t}$ is also invertible. Thus each column of $\boldsymbol{U}_{t}$ is linearly independent.   
