...
$$\mbox{Theorem 1}$$ : For a fixed $t$, $\{ \boldsymbol{U}_{t}(1), \boldsymbol{U}_{t}(2), ... , \boldsymbol{U}_{t}(n) \}$ provides a linearly independent $n$-coordinate vectors.}
...
$$\mbox{Theorem 2}$$ : Assume $E\big| \boldsymbol{U}^2 \big| < \infty$   and $E\big| \boldsymbol{X}^2 \big| < \infty$. For a fixed $t$, $\mathcal{R}_{n}(\boldsymbol{U}, \boldsymbol{X})$ (MCorr)  = 0 if and only if $\boldsymbol{U}$ and $\boldsymbol{X}$ are independent, i.e. connectivity at $t$ between nodes are independent of their attributes. 
