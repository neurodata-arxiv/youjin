...
$$\boldsymbol{U}_{t}(i) = \begin{pmatrix} \lambda^{t}_{1} \phi_{1}(i) & \lambda^{t}_{2} \phi_{2} (i)  & \cdots & \lambda^{t}_{n} \phi_{q}(i) \end{pmatrix} \in \mathbb{R}^{q}.$$

Remind that $\Phi = \Pi^{-1/2}\Phi$ and $\mathbf{Q}=\mathbf{\Psi}\mathbf{\Lambda}\mathbf{\Psi}^{T} = \mathbf{\Pi}^{1/2} \mathbf{P} \mathbf{\Pi}^{-1/2}$. 
...
It was proven that we can have consistent estimator for latent position for $Z$ of each vertex, i.e. $\exists h$ s.t. $h(A) \xrightarrow{p} \hat{Z}$ (consistently estimate latent position of each vertex). 

Instead marginal distribution of A, $f_{A} = \int_{z} f_{A|Z} \int_{x} f_{Z|X} f_{X} dx dz$. To make sure that $A_{ij}$ is iid sample from its marginal distribution, $A_{ij} \overset{i.i.d.}{\sim} f_{A}$ not from $f_{A | Z}$. 
