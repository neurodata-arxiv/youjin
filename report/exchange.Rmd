...
### Theorem of iid diffusion maps 

Assume that connected, undirected and unweigted graph $\mathbf{G}$ is a graphex, i.e. exchangeable random graph from infinite graph. Then observed diffusion maps $\{ u_{i} ; i = 1,2,... , n\}$ derived from $\mathbf{G}$ are independent and identically distributed (i.i.d.) as $\mathbf{u} \sim f_{u}$.    


#### Proof

[Exchangeabiliy of Transition Probability](#exchangeability2) proves asymptotic exchangeability of diffusion maps given exchangeable graph. This holds for infinite sequence of diffusion maps from infinite graph. 

By [de Finetti](#Finetti)'s theorem, such exchangeable diffusion maps are i.i.d. given some underlying distribution.
...
Let us fix the diffusion time step $t \in \mathbb{N}$. Assume that we are given $n$-pair of observations $\{ (u_{i}, x_{i}): i = 1,2,... , n  \}$ of $u_{i} \overset{i.i.d.}{\sim} f_{x}$ and $x_{j} \overset{i.i.d.}{\sim} f_{x}$. Then MGC applied to these pair of data is theoretically consistent against all dependent alternatives.
...
The analogue of de Finetti's theorem for exchangeable arrays is the [Aldous-Hoover Theorem](Aldous). A random array $(A_{ij})$ is jointly exchangeable if and only if it can be represented as follows : There is a random function $F : [0,1]^2 \rightarrow \mathbf{A}$ such that
...
$$\big( P_{ij} \big) = \big(  \frac{A_{ij}}{deg(i)} \big) \xrightarrow{p} \big( \frac{A_{ij}}{d_{n}}  \big) \stackrel{d}{=} \big( \frac{A_{\sigma(i) \sigma(j)}}{d_{n}} \big) \xleftarrow{p} \big( \frac{A_{\sigma(i) \sigma(j)} }{deg(\sigma(i))} \big) = \big( P_{\sigma(i) \sigma(j)} \big)$$



Note that for fixed row index $i$, $A_{ij} \sim Bernoull \big( f^{\prime}(W_{j}) \big)$ some function of $f^{\prime}$ since now a latent position of $i$, $W_{i}$ is constant. From random generation of $(W_{j})$ from Uniform distribution, $(A_{ij} : j=1,2,..., n)$ are i.i.d.. 

Thus, $deg(i) \xrightarrow{p} \frac{1}{n} d_{n} = \frac{1}{n} E\big( \sum\limits_{j=1}^{n} A_{ij} \big) = \frac{1}{n} E\big( \sum\limits_{j=1}^{n} A_{\sigma(i) \sigma(j)}  \big) = \frac{1}{n} E\big( \sum\limits_{\sigma(j)=1}^{n} A_{\sigma(i) \sigma(j)}  \big)$
...
Aldous-Hoover representation theorem applies and informs us that the graph is necessarily either dense or empty [Caron and Fox, 2014](Caron).

Completely Random Measure (CRM) specifies sampling procedure for the weights and atom locations of the measure. 

A CRM $W$ on $\mathbb{R}_{+}$ is a random measure such that for any countable number of measurable sets $V_{1}, V_{2}, ... ,$ of $\mathbb{R}_{+}$, the random variables $W(V_{1}), W_{V_{2}}, ...$ are independent and 
...
[Orbanz and Williamson](Orbanz) and many others have been studying CRM framework, especially in Bayesian non-parametric literature. They studied Poisson process, compound Poisson process and also Gamma process but we are going to focus on Poisson process. 

Exchangeability for point process on $\mathbb{R}^{2}_{+}$ is formulated as follows :

Let $h > 0$ and consider intervals $V_{i} := \big[h(i-1), hi  \big]$, $i \in \mathbb{N}$. The joint process $\mathbf{A}$ is jointly exchangeable if and only if 
...
One of the special cases of sparse and exchangeable undirected graphs can be generated from a unit-rate Poisson processes and uniform random variables. A point process is exchangeable ``if and only if`` it can be represented as a transformation of unit-rate Poisson process and uniform random variables (this is in direct analogy to the graphon transformation of uniform random variables in the Aldous-Hoover representation.). More precisely, within the Kallenberg framework, we consider that two nodes $i \neq j$ connect with probability 
$$P(a_{ij} = 1 \big| w_{i}, w_{j}) = 1 - e^{-2 w_{i} w_{j}}$$
where the positive "sociability" parameters $(w_{i})_{i=1,2,..}$ are the points of a Poisson point process. They showed that by carefully choosing the Levy measure characterizing this CRM, we are able to construct graphs ranging from sparse to dense. In particular, any Levy measure yielding an infinite activity CRM leads to sparse graphs; alternatively, finite activity CRMs, whose associated point process are in the compound Poisson process family, yield dense graphs. 
...
$$(\theta_{i}, \theta_{j}) \big| W, \vartheta_{i}, \vartheta_{j} \overset{ind}{\sim} Bernoulli \big( W\big( \vartheta{i}, \vartheta{j} \big) \big)$$
